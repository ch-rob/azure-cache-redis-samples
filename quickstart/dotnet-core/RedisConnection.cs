...
namespace Redistest
{
    public class RedisConnection : IDisposable
    {
        ...
        private readonly string _redisAccessKey;
        private readonly string _authenticationType;
        private ConnectionMultiplexer _connection;
        private IDatabase _database;

        private RedisConnection(string redisHostName, string redisAccessKey = null, string authenticationType = "WORKLOAD_IDENTITY")
        {
            _redisHostName = redisHostName;
            _redisAccessKey = redisAccessKey;
            _authenticationType = authenticationType;
        }

        public static async Task<RedisConnection> InitializeAsync(string redisHostName, string redisAccessKey = null, string authenticationType = "WORKLOAD_IDENTITY")
        {
            var redisConnection = new RedisConnection(redisHostName, redisAccessKey, authenticationType);
        ...
        private async Task ForceReconnectAsync(bool initializing = false)
        {
            ...
            try
            {
                ...
                switch (_authenticationType.ToUpperInvariant())
                {
                    case "WORKLOAD_IDENTITY":
                        Console.WriteLine($"Connecting to {_redisHostName} with workload identity...");
                        configurationOptions = await ConfigurationOptions.Parse($"{_redisHostName}:10000").ConfigureForAzureWithTokenCredentialAsync(new DefaultAzureCredential());
                        break;
                        
                    case "ACCESS_KEY":
                    case "ACCESSKEY":
                        Console.WriteLine($"Connecting to {_redisHostName} with access key...");
                        configurationOptions = ConfigurationOptions.Parse($"{_redisHostName}:10000,password={_redisAccessKey},ssl=true");
                        break;
                        
                    default:
                        throw new InvalidOperationException($"Invalid authentication type: {_authenticationType}");
                }
